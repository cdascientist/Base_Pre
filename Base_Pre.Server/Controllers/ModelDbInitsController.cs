...
namespace Base_Pre.Server.Controllers
{
    ...
    public class ModelDbInitsController : ControllerBase
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss.fff}] Session {sessionId}: Starting ProcessFactoryOne");
            try
            {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss.fff}] Session {sessionId}: Error in ProcessFactoryOne: {ex.Message}");
                throw;
            }
        }
    ...
    Session session, ConcurrentDictionary<string, object> results)
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    using (var sess = tf.Session(g))
                    {
                        ...
                        // Different training data for Model B
                        var priceData = new float[,] {
                    { 150f/1000f },
                    { 250f/1000f },
                    { 350f/1000f }
                        ...
                        for (int i = 0; i < 3; i++)
                        {
                            oneHotNames[i, (i + 1) % 3] = 1.0f;  // Different pattern for Model B
                        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss.fff}] Session {sessionId}: Starting ProcessFactoryFour");
            try
            {
                System.Diagnostics.Debug.WriteLine($"[{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss.fff}] Session {sessionId}: Saving results to database");
                await _context.SaveChangesAsync();

                System.Diagnostics.Debug.WriteLine($"[{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss.fff}] Session {sessionId}: ProcessFactoryFour completed successfully");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss.fff}] Session {sessionId}: Error in ProcessFactoryFour: {ex.Message}");
                throw;
            }
        }
    }
